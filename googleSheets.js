...
const deleteUserDataSheet = async (username) => {
    const spreadsheetInfo = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
...
const handleCodeSubmission = async (username, codeType, codes) => {
    ...
    const uniqueNewCodes = [], duplicateCodes = [], invalidFormatCodes = [];
...
const aggregateUserBatches = async (username) => {
    ...
    allData.slice(1).forEach(row => {
        ...
        if (batchId) {
            if (!batchData[batchId]) batchData[batchId] = { type: row[typeCol], count: 0, statusCounts: {} };
    ...
    for (const batchId in batchData) {
        ...
        else if (batch.statusCounts['Paid'] === batch.count) batch.status = 'Paid';
        else if (batch.statusCounts['Paid']) batch.status = 'Partially Paid';
...
const getBatchDetails = async (username, batchId) => {
    ...
    const headers = getHeaders(allData);
    const batchIdCol = findCol(headers, 'batch id'), codeCol = findCol(headers, 'code'), typeCol = findCol(headers, 'type'), statusCol = findCol(headers, 'status');
    ...
    allData.slice(1).forEach(row => {
        if (String(row[batchIdCol]) === String(batchId)) {
            ...
            details.status = row[statusCol] || 'Pending'; // Simplified status
        }
    });
...
const calculateUserFinancials = async (username) => {
    const allData = await getSheetData(username);
    const financials = { totalNetOwed: 0, unpricedCount: 0, totalSubmissions: 0, typeStats: {} };
    if (allData.length <= 1) return financials;

    const headers = getHeaders(allData);
    const priceCol = findCol(headers, 'price'), statusCol = findCol(headers, 'status'), typeCol = findCol(headers, 'type');
    ...
    allData.slice(1).forEach(row => {
        ...
        if (!financials.typeStats[type]) financials.typeStats[type] = { priced: 0, unpriced: 0, total: 0 };
